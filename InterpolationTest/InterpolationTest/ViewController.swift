...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let image1 = UIImage(data: data1) {
            if let image2 = UIImage(data: data2) {
                imageView?.animationImages = generateInterpolatedImages(fromImage: image1, toImage: image2, num: 10)
    ...
    func generateInterpolatedImages(fromImage: UIImage, toImage: UIImage, num: Int) -> [UIImage] {
        var images = [UIImage]()
        guard num > 0 else {
            return images
        }
        images.append(fromImage)
        for i in 1...num {
            let ratio = Float(i) / Float(num+1)
            if let interpolatedImage = interpolateImage(img1: fromImage, img2: toImage, ratio: ratio) {
